...
All blocks below are yrXlsim YAML from the spec. They are rendered into Excel-style tables by JavaScript when you build the book to HTML.
...
*Note: The renderer shows FORMULAS view; `values` override is for VALUES view and is not applied in this display.*
...
You can open this project in RStudio and use **Build** → **Render Book** to generate the HTML. The rendered book shows the original YAML source above each Excel-like spreadsheet preview (one code block per example — no repetition).
